...
/**
 * @description Componente de la bandeja de entrada para registrar cheques.
 * Permite gestionar hasta 30 cheques y muestra una advertencia cuando se alcanza el límite.
 * @returns {JSX.Element} Formulario de ingreso de cheques y validación del límite.
 */
export default function Inbox() {
  ...
  /**
   * @description Maneja el envío del formulario para registrar un cheque.
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("/api/cheques", {
        method: "POST",
        body: JSON.stringify({ bankName, date, dueDate, amount, checkNumber, holder, bandeja: "entrada" }),
  ...
  return (
    <div className="p-8 relative min-h-screen flex flex-col items-center">
      <h1 className="text-3xl font-bold text-green-600 mt-6">Bandeja de Entrada</h1>
      ...
      {/* ✅ Formulario para agregar cheques */}
      <form className="mt-4 w-full max-w-lg" onSubmit={onSubmit}>
        ...
        <div className="mb-4">
          <label htmlFor="holder" className="block text-sm font-medium">Titular</label>
          <input type="text" id="holder" name="holder" className="mt-1 p-2 border rounded-md w-full" placeholder="Nombre del titular" required />
        </div>
